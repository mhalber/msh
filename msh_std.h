/*
  ...
  ==================================================================================================
  TODOs:
  [ ] Memory allocation / alternative alocators
...
#define MSH_PLATFORM_NAME "asm.js "         \
    MSH_STRINGIFY(__EMSCRIPTEN_major__) "." \
    MSH_STRINGIFY(__EMSCRIPTEN_minor__) "." \
    MSH_STRINGIFY(__EMSCRIPTEN_tiny__)
...
#define MSH_COMPILER_NAME "GCC "      \
    MSH_STRINGIFY(__GNUC__) "."       \
    MSH_STRINGIFY(__GNUC_MINOR__) "." \
    MSH_STRINGIFY(__GNUC_PATCHLEVEL__)
#elif MSH_COMPILER_CLANG
#define MSH_COMPILER_NAME "Clang "     \
    MSH_STRINGIFY(__clang_major__) "." \
    MSH_STRINGIFY(__clang_minor__) "." \
    MSH_STRINGIFY(__clang_patchlevel__)
...
MSH_STD_DEF double   msh_time_diff(int32_t unit, uint64_t new_time, uint64_t old_time);
...
MSH_STD_DEF void msh_print_progress_bar(char* prefix, char* suffix,
...
MSH_STD_DEF size_t      msh_strcpy_range(char* dst, const char* src, size_t start, size_t len);
...
MSH_STD_DEF void    msh_distrib2pdf(const double* dist, double* pdf, int32_t n_vals);
MSH_STD_DEF void    msh_pdf2cdf(const double* pdf, double* cdf, int32_t n_vals);
MSH_STD_DEF void    msh_invert_cdf(const double* cdf, int32_t n_vals,
                              double* invcdf, int32_t n_invcdf_bins);
MSH_STD_DEF int     msh_pdfsample_linear(const double* pdf, double prob, int32_t n_vals);
MSH_STD_DEF int     msh_pdfsample_invcdf(const double* pdf, double prob, int32_t n_vals);
