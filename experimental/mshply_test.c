...
typedef struct ply_property
{
  ...
  int type;      // Type that property stores
  int list_type; // Type that list counter is stored 

  int byte_size;           // Byte size of a property
  int counter_byte_size;   // Byte size of a list counter
  
  int count;               // In list, how many properties there are
...
int  ply_file_get_element(ply_file_t* pf, const char* element_name, ply_element_t** elem);
...
int  ply_file_add_property(ply_file_t* pf, 
...
ply__parse_property_cmd(char* line, ply_file_t* pf)
{
  ...
  if (sscanf(line, "%s %s %s %s %s", &cmd[0], &list_str[0], &list_type_str[0], &type_str[0], (char*)&pr.name) == 5)
  {
    ...
    ply__string_to_property_type(type_str, &pr.type, &pr.byte_size);
    ply__string_to_property_type(list_type_str, &pr.list_type, &pr.counter_byte_size);
    pr.count = -1;//UNKNOWN (TODO: Add maybe enum to indicate this?)
    valid_format = true;
  }
...
int 
ply_file__calculate_elem_size_ascii(ply_file_t* pf, ply_element_t* elem)
{
  char line[PLY_MAX_STR_LEN];
  while(fgets(&line[0], PLY_MAX_STR_LEN, pf->_fp))
  {
    ...
    for(int j=0; j<msh_array_size(elem->properties); ++j)
    {
      ply_property_t* prop = &elem->properties[j];
      while(*cp_up!=' ' && *cp_up!='\n') {cp_up++;}
      char tmp_cp_up = *cp_up;
      *cp_up = 0; //Fake string end;

      if(prop->list_type != PLY_INVALID)
      {
        switch(prop->list_type)
        {
          ...
          case PLY_UINT8:  
          case PLY_UINT16: 
          case PLY_UINT32: prop->count = (int32_t)atoi(cp_low);  break;
          case PLY_FLOAT:
          case PLY_DOUBLE: prop->count = (int32_t)atof(cp_low);  break;
        }
        // skip
        *cp_up = tmp_cp_up;
        cp_low = cp_up;
      ...
      *cp_up = tmp_cp_up;
      ...
      prop->total_byte_size += prop->counter_byte_size + prop->count * prop->byte_size;
      prop->total_count += prop->count;
    }
  }
  printf("------------\n");
  return PLY_NO_ERRORS;
...
int 
ply_file__calculate_elem_size_le(ply_file_t* pf, ply_element_t* elem)
{
  ...
  for(int i=0; i < elem->count; ++i)
  {
    for(int j=0; j < msh_array_size(elem->properties); ++j)
    {
      ply_property_t* prop = &elem->properties[j];
      int count = 1;
      if( prop->type != PLY_INVALID ) 
      { 
        if( fread(&count, prop->counter_byte_size, 1, pf->_fp ) != 1) { return PLY_PARSE_ERROR;}
        prop->total_byte_size += prop->counter_byte_size;
      }
      fseek(pf->_fp, count * prop->byte_size, SEEK_CUR);
      prop->total_byte_size += count * prop->byte_size;
      prop->total_count += count;
    }
  }
  return PLY_NO_ERRORS;
}
...
ply_file_parse_contents(ply_file_t* pf)
{
  ...
  {
    ply_element_t* elem = &pf->elements[i];
    int n_properties = msh_array_size(elem->properties);
    elem->file_anchor = ftell(pf->_fp);

    // Determine if any of the properties in the element has list
    int can_precalculate_size = 1;
    for(int j=0; j<n_properties; ++j)
    {
      if( elem->properties[j].list_type != PLY_INVALID ) 
      { 
        int has_hint = 0;
        for(int k=0; k<msh_array_size(pf->_hints); ++k)
        {
          if( !strcmp(elem->properties[j].name, pf->_hints[k].property_name) ) 
          { 
            has_hint = 1;
            elem->properties[j].count = pf->_hints[k].expected_size;
          };
        }
        if(!has_hint)
        {
    ...
    if(can_precalculate_size)
    {
      ...
      {
        ply_property_t* prop = &elem->properties[j];
        prop->total_byte_size = prop->byte_size * prop->count * elem->count;
        elem_size += prop->byte_size * prop->count; 
        if(prop->list_type != PLY_INVALID )
        {
          prop->total_byte_size += prop->counter_byte_size*elem->count;
          elem_size += prop->counter_byte_size;
        }
        prop->total_count += prop->count * elem->count;
      }
      if(pf->format != PLY_ASCII) { fseek(pf->_fp, elem->count * elem_size, SEEK_CUR); }
      else
      {
        char line[PLY_MAX_STR_LEN];
        for(int j=0; j<elem->count; ++j) { fgets(&line[0], PLY_MAX_STR_LEN, pf->_fp); }
      }
    }
    else 
    {
      // There exists a list property. We need to calculate required size via pass through
      if(pf->format == PLY_ASCII )         err_code = ply_file__calculate_elem_size_ascii(pf, elem);
      if(pf->format == PLY_LITTLE_ENDIAN ) err_code = ply_file__calculate_elem_size_le(pf, elem);
    ...
    for(int j = 0 ; j < msh_array_size(pf->elements[i].properties); ++j)
    {
      ...
      else
      {
        ...
        printf("      '%s'%s | %7s(%2d bytes) %7s(%2d bytes)\n", 
                                        ...
                                        list_type, pf->elements[i].properties[j].counter_byte_size,
...
int
ply_file_get_element(ply_file_t* pf, const char* element_name, ply_element_t** elem)
{
  int err_code = PLY_NO_ERRORS;
  ...
  *elem = NULL;
  for(int i=0;i<msh_array_size(pf->elements);++i)
  {
    if(!strcmp(pf->elements[i].name, element_name)) { *elem = &pf->elements[i]; break; }
  }
  if( !*elem ) return PLY_ELEMENT_NOT_FOUND_ERR;
                      ...
                      ply_property_t** prop)
{
  int err_code = PLY_NO_ERRORS;
  // find requested element
  ply_element_t* elem = NULL;
  err_code = ply_file_get_element(pf, element_name, &elem);
  if( err_code ) return err_code;

  *prop = NULL;
  for(int i=0;i<msh_array_size(elem->properties);++i)
  {
    if(!strcmp(elem->properties[i].name, property_name)) { *prop = &elem->properties[i]; break; }
  }
  if( !*prop ) return PLY_PROPERTY_NOT_FOUND_ERR;
...
ply_file_get_element_count(ply_file_t* pf, const char* element_name, int* count)
{
  ...
  for(int i=0;i<msh_array_size(pf->elements);++i)
  {
    if(!strcmp(pf->elements[i].name, element_name)) elem = &pf->elements[i];
  }
  if( !elem ) return PLY_ELEMENT_NOT_FOUND_ERR;
...
ply_file_get_element_size(ply_file_t* pf, const char* element_name, int* size)
{
  ...
  *size = 0;
  for(int i= 0; i<msh_array_size(elem->properties); ++i)
  {
    *size += elem->properties[i].total_byte_size;
  }
  return err_code;
}
...
ply_file__get_element_data_ascii(ply_file_t* pf, const char* element_name, void** storage)
{
  ...
  fseek(pf->_fp, elem->file_anchor, SEEK_SET);
  ...
  for(int i=0;i<elem->count;++i)
  {
    ...
    for(int j=0; j<msh_array_size(elem->properties); ++j)
    {
      ...
      char tmp_cp_up = *cp_up;
      *cp_up = 0; //Fake string end;
      if(prop->list_type == PLY_INVALID) // regular prop
      { 
        switch(prop->type)
        {
                             ...
                             const char** properties_names, int n_properties, int* size)
{
  ...
  for( int i=0;i<n_properties;++i)
  {
    for( int j=0;j<msh_array_size(elem->properties);++j)
    {
      if( !strcmp(elem->properties[j].name, properties_names[i]) )
      {
        n_found++;
        total_byte_size += elem->properties[j].total_byte_size;
      } 
    }
  }
                                        ...
                                        void** list_lengths, void** list_data)
{
  ...
  {  
    for(int j=0; j<msh_array_size(properties); ++j)
    {
      ...
      {
        memcpy(ll_dst, src, properties[j].counter_byte_size);
        uint64_t count = 0;
        if( properties[j].count != -1)
        {
          count = properties[j].count;
        }
        else
        {
        ...
        memcpy(ld_dst, src+properties[j].counter_byte_size, count * properties[j].byte_size);
        src += properties[j].counter_byte_size + count * properties[j].byte_size; 
        ll_dst += properties[j].counter_byte_size;
...
int  
ply_file_add_property(ply_file_t* pf, const char* element_name, const char* property_name, int property_type)
{
	// Find element
  ply_element_t* el = NULL;
  for(int i = 0; i < msh_array_size(pf->elements); ++i)
  ...
  { 
    ply_property_t prop;
    strncpy(&prop.name[0], property_name, 64);
    prop.type = property_type;
    prop.list_type = PLY_INVALID;
      switch( property_type )
      {
        case PLY_INT8:   {  prop.byte_size = 1; break; }
        case PLY_INT16:  {  prop.byte_size = 2; break; }
        case PLY_INT32:  {  prop.byte_size = 4; break; }
        case PLY_UINT8:  {  prop.byte_size = 1; break; }
        case PLY_UINT16: {  prop.byte_size = 2; break; }
        case PLY_UINT32: {  prop.byte_size = 4; break; }
        case PLY_FLOAT:  {  prop.byte_size = 4; break; }
        case PLY_DOUBLE: {  prop.byte_size = 8; break; }
      }
    prop.counter_byte_size = 0;
    prop.count = 1;
    prop.total_byte_size = prop.byte_size + prop.counter_byte_size;
    printf("TEST: %d %d\n", prop.byte_size, prop.total_byte_size);
    msh_array_push( el->properties, prop );
  }
  else 
  {
...
int  
ply_file_add_list_property(ply_file_t* pf, 
             const char* element_name, const char* property_name, int list_type, int property_type)
{
	// Find element
  ply_element_t* el = NULL;
  ...
  { 
    ...
    switch( list_type )
    {
      case PLY_INT8:   {  prop.counter_byte_size = 1; break; }
      case PLY_INT16:  {  prop.counter_byte_size = 2; break; }
      case PLY_INT32:  {  prop.counter_byte_size = 4; break; }
      case PLY_UINT8:  {  prop.counter_byte_size = 1; break; }
      case PLY_UINT16: {  prop.counter_byte_size = 2; break; }
      case PLY_UINT32: {  prop.counter_byte_size = 4; break; }
      case PLY_FLOAT:  {  prop.counter_byte_size = 4; break; }
      case PLY_DOUBLE: {  prop.counter_byte_size = 8; break; }
    }
...
int 
ply_file_write_header(ply_file_t* pf)
{
  ...
  char *format_str = NULL;
  char type_str[8] = {0};
  char list_type_str[8] = {0};
  if(pf->format == PLY_ASCII)         { format_str = "ascii"; } //RETURN UNSUPPORTED?
  if(pf->format == PLY_LITTLE_ENDIAN) { format_str = "binary_little_endian"; } 
  if(pf->format == PLY_BIG_ENDIAN)    { format_str = "binary_big_endian"; }// RETURN UNSUPPORTED?
  fprintf(pf->_fp, "format %s %2.1f\n", format_str, (float)pf->format_version);

  for(int i = 0 ; i < msh_array_size(pf->elements); ++i)
  {
    fprintf(pf->_fp, "element %s %d\n", pf->elements[i].name, pf->elements[i].count);
    for(int j = 0 ; j < msh_array_size(pf->elements[i].properties); ++j)
    {
      switch( pf->elements[i].properties[j].type )
      {
        case PLY_INT8:   { snprintf(type_str, 7, "char"); break; }
        case PLY_INT16:  { snprintf(type_str, 7, "short"); break; }
        case PLY_INT32:  { snprintf(type_str, 7, "int"); break; }
        case PLY_UINT8:  { snprintf(type_str, 7, "uchar"); break; }
        case PLY_UINT16: { snprintf(type_str, 7, "ushort"); break; }
        case PLY_UINT32: { snprintf(type_str, 7, "uint"); break; }
        case PLY_FLOAT:  { snprintf(type_str, 7, "float"); break; }
        case PLY_DOUBLE: { snprintf(type_str, 7, "double"); break; }
      }

      if(pf->elements[i].properties[j].list_type == PLY_INVALID)
      {
        fprintf(pf->_fp, "property %s %s\n", type_str, pf->elements[i].properties[j].name);
      }
      else
      {
        switch( pf->elements[i].properties[j].list_type )
        {
          case PLY_INT8:   { snprintf(list_type_str, 7, "char"); break; }
          case PLY_INT16:  { snprintf(list_type_str, 7, "short"); break; }
          case PLY_INT32:  { snprintf(list_type_str, 7, "int"); break; }
          case PLY_UINT8:  { snprintf(list_type_str, 7, "uchar"); break; }
          case PLY_UINT16: { snprintf(list_type_str, 7, "ushort"); break; }
          case PLY_UINT32: { snprintf(list_type_str, 7, "uint"); break; }
          case PLY_FLOAT:  { snprintf(list_type_str, 7, "float"); break; }
          case PLY_DOUBLE: { snprintf(list_type_str, 7, "double"); break; }
        }
        fprintf(pf->_fp, "property list %s %s %s\n", 
                                      list_type_str, type_str, pf->elements[i].properties[j].name);
      }
    }
  }
...
int  
ply_file_write_element_data( ply_file_t* pf, const char* element_name,  void* data )
{
  if( !pf->_fp ) { return PLY_INVALID_FILE_ERR; }
  if( pf->format==PLY_ASCII ) { return PLY_FORMAT_UNRECOGNIZED_ERR;} // CHANGE TO UNSUPPORTED
  else if( pf->format==PLY_BIG_ENDIAN ) { return PLY_FORMAT_UNRECOGNIZED_ERR; }
  else
  {
    // Find element // TODO(maciej): MAKE FIND ELEMENT ITS OWN FUNCTION AND ADD ERROR
    ply_element_t* el = NULL;
    for(int i = 0; i < msh_array_size(pf->elements); ++i)
    {
      if(!strcmp(element_name, pf->elements[i].name))
    ...
    if(el)
    {
      // Check if it contains list properties and then decide on a path
      int32_t data_size = 0;
      for(int j = 0 ; j < msh_array_size(el->properties); ++j)
      {
        if(el->properties[j].list_type == PLY_INVALID)
        {
          data_size += el->count * el->properties[j].total_byte_size;
        }
      }
      fwrite( data, 1, data_size, pf->_fp );
    }
  }
  return PLY_NO_ERRORS;
}
...
int  
ply_file_write_element_data( ply_file_t* pf, const char* element_name,  void* data, void* indices_data )
{
  // TODO(maciej): This will need to combine the indices count and actual data.
  // Check how tinyply deals with that.
  return PLY_NO_ERRORS
}
...
int main(int argc, char** argv)
{
  ...
  ply_file_add_element(pf, "vertex", 3 );
  ply_file_add_property(pf, "vertex", "x", PLY_FLOAT );
  ply_file_add_property(pf, "vertex", "y", PLY_FLOAT );
  ply_file_add_property(pf, "vertex", "z", PLY_FLOAT );

  // ply_file_add_element(pf, "face", 1 );
  // ply_file_add_list_property(pf, "face", "vertex_indices", PLY_UINT8, PLY_INT32 );

  ply_file_write_header(pf);  
  ply_file_write_element_data(pf, "vertex", {"x", "y", "z"}, vertices );// I THINK I LIKE PASSING PROPERTIES LIKE THAT.
  ...
  ply_file_add_hint(pf, indices_size_hint);
    ...
    unsigned char* ind_count = (unsigned char*)malloc(face_count * ind_prop->counter_byte_size);  
